...
describe("Route for personal access: loading data", function() {

  function testExistence(columnName) {
    describe("testing loaded " + columnName, function() {
    let hardcoded = _.get(fixtures, ${columnName}N )

      return mongo
        .count(_.get(nconf.get('schema'), columnName),
          { userId: fixtures.staticUserId })
        .then(function(amount) {
            expect(amount).to.equal(hardcoded);
        });
  };

  return fixtures
    .loadFixtures()
    .tap(testExistence('timelines'))
    .tap(testExistence('impressions'))
    .tap(testExistence('metadata'))
    .tap(testExistence('htmls'));
