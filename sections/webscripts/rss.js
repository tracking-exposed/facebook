...
function buildRSS(lang, e, ui) {
    const escaped = encodeURIComponent(ui.item.value);
    $("#status").html(`
        <img style="width:20px" src="${rssIcon}">
        <span class="monospace">
            https://facebook.tracking.exposed/feeds/0/${lang}/${escaped}
        </span>
    `);
};
...
function configureAutocomplete(lang) {
    $("#inputbar").autocomplete({
        ...
        autoFocus: true,
        minLength: 0,
        select: _.partial(buildRSS, lang)
    });
    $("#inputbar").on("autocompleteselect", _.partial(buildRSS, lang));
}
