...
function setMongoUri(forced) {
    ...
    mongoUri = forced ? forced : `mongodb://${mongoHost}:${mongoPort}/${mongoDb}`;
    debug("Initializing mongoUri at %s", mongoUri);
}
...
function clientConnect(config) {
    ...
    return new Promise((resolve, reject) => (
        MongoClient.connect(mongoUri, {
            ...
            if(err) {
                console.error("mongo.clientConnect:", err);
                reject(err);
            } else {
                resolve(client);
            }
...
function writeMany(mongoc, cName, docs) {
    return mongoc
        .db()
        .collection(cName)
        .insertMany(docs);
...
function remove(mongoc, cName, selector) {
