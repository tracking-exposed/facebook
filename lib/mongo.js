...
var upsertOne = function(cName, selector, updated) {
    var p = performa.begin('upsertOne',  cName, {
        'selector': selector
    });
    return Promise.using(dbConnection(), function(db) {
        return db
...
function read(cName, selector, sorter) {
    ...
    .tap(function(rv) {
        /*
        debug("read in %s by %j sort by %j → read %d objects", 
...
var readLimit = function(cName, selector, sorter, limitN, past) {
    ...
    debug("readLimit in %s by %j limit %d skip %d", 
        cName, selector, limitN, past);
    return Promise.using(dbConnection(), function(db) {
        return db
            .collection(cName)
            .find(selector)
...
function lookup(cName, query, sequence) {
    ...
    var p = performa.begin('lookup', cName, {
        query: query,
        sequence: sequence
    });
