    ...
    return Promise.using(dbConnection(), function(db) {
        return db
            ...
            .then(function(results) {
                debug("result writeOne %j", results);
                return true;
            })
    ...
    return Promise.using(dbConnection(), function(db) {
        return db
            ...
            .then(function(result) {
                debug("result upsertOne %j", result);
                return true;
            })
            .catch(function(error) {
                debug("upsetOne Error %s (%j)", cName, error);
                return false;
            });
    });
};
...
var writeMany = function(cName, dataObjects) {
    debug("upsertMany in %s of %d objects", cName, _.size(dataObjects));
    return Promise.using(dbConnection(), function(db) {
        return db
            .collection(cName)
            .insertMany(dataObjects, { uspert: true})
            .then(function(results) {
                return true;
            })
            .catch(function(error) {
