...
var countriesStats = function(req) {
    ...
    debug("%s countriesStats with format '%s'", req.randomUnicode, format);
...
var byDayPostType = function(req) {
    /* it is clear this is not they way, but this is just legacy alpha */
    debug("%s byDayPostType", req.randomUnicode);

    var column = nconf.get('schema').impressions;
    var activitySources = [
        { 'name': "Public",
          'filter': 'public',
        ...
        { 'name': "Non-public",
          'filter': 'private',
          'description': "Friends only and restricted audience"
        }
    ];

    return Promise.map(activitySources, function(aS) {
        return mongo
            .countByDay(column, '$impressionTime', { visibility: aS.filter });
    })
...
var byDayActivity = function(req) {
    ...
    var activitySources = [
        ...
        { 'name': "timelines",
          ...
          'timeVarName': "$startTime",
          'description': "Number of facebook' Feed refreshes recorded"
        },
        { 'name': "impressions",
          'column': nconf.get('schema').impressions,
          'timeVarName': "$impressionTime",
          'description': "Number of posts impression reported"
        },
        { 'name': "htmls",
          'column': nconf.get('schema').htmls,
          'timeVarName': "$savingTime",
          'description': "Number of HTML snippet to be parsed"
        },
        { 'name': "accesses",
          'column': nconf.get('schema').accesses,
          'timeVarName': "$when",
          'description': "Website pages accesses"
        }
    ];
...
function manualBoarding(req) {
    ...
    debug("%s manualBoarding user %d pubKey %s",
