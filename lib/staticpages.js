...
var pageMap = {
  ...
  'new': jadeCompiler('new'),
  'old-homepage': jadeCompiler('homepage'),
  '404': jadeCompiler('404'),
  'INDEX': jadeCompiler('next')
};
...
var getPage = function(req) {
    ...
    if(_.isUndefined(pageName)) {
        pageName = 'INDEX';
        debug("%s getPage of /", req.randomUnicode);
    }
    else if(_.isUndefined(_.get(pageMap, pageName))) {
