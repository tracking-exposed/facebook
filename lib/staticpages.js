...
var impact = jadeCompiler('impact');
...
var getPersonal = function(req) {
    ...
    return Promise.all([
      mongo.read(nconf.get('schema').supporters, {userId: profileId}),
      mongo.count(nconf.get('schema').timeline, {userId: profileId})
    ])
    .then(function(infos) {
        user = _.first(infos);
        timelinfo = _.last(infos);

        if(_.isUndefined(user) || _.isUndefined(user.userId)) {
            debug("user %d not found in the supporters list", profileId);
            return {
                'profileId': 0,
        ...
        debug("TODO Take the number of timelines: %j", timelinfo);
        userInfo = {
