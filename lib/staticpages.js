...
var staticAccess = function(request, details) {
    ...
    if(!_.isUndefined(details, 'page')) {
        debug("%s logging of %j lack of 'page' key",
            request.randomUnicode, details);
    }
    debug("%s Logging access of %j", request.randomUnicode, details);
...
var shapePersonal = function(req, userId) {
    ...
    return Promise.all([
        ...
        mongo.countByObject(nconf.get('schema').timeline, {userId: userId})
    ])
    .then(function(infos) {
...
var getRealityMeter = function(req) {
    ...
    return staticAccess(req, {page: "realitymeter"})
        .then(function() {
            return mongo.countByObject(nconf.get('schema').supporters);
        })
