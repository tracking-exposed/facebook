...
module.exports = {
    ...
    viralcheck: require('../parsers/viralcheck'),
    nature: require('../parsers/nature'),
    textsearch: require('../parsers/textsearch'),
...
async function getLastHTMLs(filter, amount) {
    ...
    const htmls = await mongo3.aggregate(mongodrivers.readc,
        nconf.get('schema').htmls, [
            { $match: filter },
            // ricorda questo era savingTime, ma l'indice era su impressionTime
            // potrebbero crearsi situazioni limite se un client ha l'orario
            // sminchiato. meglio sarebbe mettere savingTime come indice.
