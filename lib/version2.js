...
var slidesP = jade.compileFile(
    __dirname + '/../sections/moca.jade',
...
var postFeed = function(req) {
    ...
    /* it is happen than some postId are 'null', why ? */
    if(_.size(_.find(processed.timeline, function(p) { 
        return _.isNaN(p.postId); 
    })) && false ) {
        debug("%s An anomaly has been spot, saving log", req.randomUnicode);
        functionList.push(
            utils.JSONsave('/dev/shm', 
                    'anomaly-' + supportInfo.numId + 
