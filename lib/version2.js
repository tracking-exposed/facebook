...
var byDayActivity = function(req) {
    ...
    .then(function(unassociated) {
        return _.map(unassociated, function(lists, ndx) {
            ...
            /* special, only in this case, cut off the last day */
            if(reference.name === "supporters" && _.size(filter) === 0) {
...
var processedUserLog = function(req) {
    ...
    switch(kind) {
        case 'distortion':
            var apromise = analysis.feedDistortion(filter);
            break;
        case 'presence':
    ...
    return Promise
        ...
        .then(function(ret) {
            if(format === 'column') {
                var convertor = {
                    'distortion': utils.distortionToColumn,
                    'presence': utils.presenceToColumn,
                    'absolute': utils.absoluteToColumn
                };
