...
var topPosts = function(req) {
    ...
    return mongo
        .readTopPost( nconf.get('schema').timeline, 0, 200)
        .then(function(results) {
            /* here the dirty data get cleaned */
            return _.reduce(results, function(memo, pe) {
                if( _.eq( _.size(memo), EXAMPLES_LIMIT) )
                    return memo;
...
/* This is an *experiment* because the mongo query returns a number
 ...
 * so I'm doing the filtering with JS */
var topPostsEXP = function(req) {
    var EXAMPLES_LIMIT = 20;
    debug("%s TopPostsEXP", req.randomUnicode);
    return mongo
        .experimentalTP( nconf.get('schema').timeline)
        .then(function(results) {
            /* here the dirty data get cleaned */
            return _.reduce(results, function(memo, pe) {
