...
var nodeInfo = function(req) {
    ...
    return Promise
        ...
        .then(function(numbers) {
            ...
            var columnsSizes = {
                supporters: numbers[0][0].count,
                timeline: numbers[1][0].count,
                refreshes: numbers[2][0].count
            };

            debug("nodeInfo will cause %d iterations",
                sharding.estimate(columnsSizes));
            return {
                json: {
...
var byDayPostType = function(req) {
    ...
    var column = nconf.get('schema').timeline;
    var activitySources = [
        { 'name': "promoted",
        ...
        { 'name': "feed",
          'timeVarName': "$displayTime",
          'description': "Friends and followed feed"
        },
        { 'name': "friendlink",
          'timeVarName': "$displayTime",
          'description': "Post with friends activity"
        },
    ];

    return Promise.map(activitySources, function(aS) {
        return mongo.countByDay(column, "$displayTime", { type: aS.name});
    })
