...
function importSelector(body) {
    ...
    if(_.isString(repetition)) {
        debug("Fuck moment, fuck fuck fuck");
    ...
    debug("Parser %s query %s", ret.parserName, JSON.stringify(ret.mongoQuery));
...
function snippetResult(req) {
    ...
    return mongo
        .readLimit(nconf.get('schema').htmls, { 'id': received.htmlId },
                    { "savingTime": -1 }, htmlMaxAmount, 0)
