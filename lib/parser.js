            ...
            var sanitized = _.reduce(received.fields, function(memo, f) {
                if(_.gte(fcontent.fields.indexOf(f), 0)) {
                    ...
                    if(_.isUndefined(value))
                        debug("field %s declared but not found [%s]", f, snippet.id);
                    else if(value === "true")
                        value = true;
                    else if(value === "false")
                        value = false
...
function importSelector(body) {
    ...
    debug("until %s since %s", until, since);
...
function snippetAvailable(req) {
    ...
    return mongo
        ...
        .then(function(result) {
            ...
            return { json: {
              'available': result,
              'limit': nconf.get('htmlMaxAmount') ? _.parseInt(nconf.get('htmlMaxAmount')) : null
            }};
        });
};
...
function snippetContent(req) {
    var userReq = importSelector(req.body);
    var htmlMaxAmount = _.isString(nconf.get('htmlMaxAmount')) ? 
        _.parseInt(nconf.get('htmlMaxAmount')) : 
        nconf.get('htmlMaxAmount');

    return mongo
        .readLimit(nconf.get('schema').htmls, userReq.mongoQuery,
                   { "savingTime": -1 }, htmlMaxAmount, 0)
        .then(function(htmls) {
