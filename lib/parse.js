...
function initialize(impression) {
    /* remind: xpath and xmldom.DOMParser were initialized here.
     *
...
async function parseHTML(htmlfilter, repeat) {
    ...
    const found = mongo.read(mongoc, nconf.get('schema').htmls, htmlfilter);
    if(_.size(found) > 0)
        return found;
    ...
    /* try to retrieve remotely the HTML snippet */
    return glue
        .retrive(htmlfilter)
        .then(glue.writers)
        .tap(function(x) {
