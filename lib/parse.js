...
function finalize(envelope) {
    ...
    const errors = _.reduce(envelope, function(memo, e) {
        if(!_.size(e.errors))
            return memo;
        var r = {
            errors: e.errors,
    ...
    return {
        metadata: _.map(envelope, function(e) {
            return _.extend(
                _.pick(e.impression, impressionFields),
                _.omit(e, removef),
            );
        }),
        summary: _.map(envelope, summarize),
        errors
    };
}
        ...
        .tap(function(impressions) {
            if(_.size(impressions)) {
                ...
                const humanized = moment
                    ...
                    .humanize();
                debug("Processing %d impressions: %s [%s] %s %s [%s] %s",
                    /*                                    ^^^^^^^^^^^^^ conditionals */
                    _.size(impressions),
