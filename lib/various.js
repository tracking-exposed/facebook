...
function loadJSONfile(fname) {
    ...
    return fs
        ...
        .tap(function(check) {
            if(!_.size(check))
                throw new Error("File " + fname + " is emprty");
        });
};

function dumpJSONfile(fname, content) {
    const stringified = JSON.stringify(content, undefined, 1);
    debug("Opening file %s for write there %d bytes", fname, _.size(stringified));
    return fs
        .writeFileAsync(fname, stringified, "utf-8");
};
