...
function buildXML(lang, label, enriched) {
    ...
    const composed = new RSS({
        ...
        feed_url: `https://facebook.tracking.exposed/feeds/0/${lang}/${label}`,
        ttl: 60
    });
...
function appendItem(e) {
    ...
    let permaLink =  summary.permaLink ? summary.permaLink : postIdLink;
    permaLink = _.startsWith(permaLink, 'http') ? permaLink : `https://facebook.com${postIdLink}`
    if(!permaLink.match(/k\.com\//)) {
        debug("Error condition: permaLink is %s", permaLink);
        permaLink = null;
    }
...
function produceError(lang, label, note) {
    const url = lang ?
        `https://facebook.tracking.exposed/feeds/0/${lang}/${label}` :
