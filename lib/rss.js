...
function composeXMLfromFeed(feed) {
    /* this is the high level function call by buildrsserv */
    const timelimit = new Date(moment().subtract(5, 'd'));
    /* timelimit is hardcoded but can come from feed or from other source */
    return retrieveNewData(feed, timelimit)
    /* retrieveNewData is the function fetching the metadata from the feed */
        .then(function(blob) {
            /* mergeSemantciBlock might be refactored/improved when the function above is
             * re-engineer to let people experiment more algorithms */
            return mergeSemanticLabel(feed, blob);
        })
        .then(composeXML);
    /* this function return a complex accumulated data which get saved after by saveRSSfile */
}


function mergeSemanticLabel(feed, blob) {
...
function composeXML(material) {
    ...
    material.composed = composed;
    return material;
}

function saveRSSfile(material) {
...
function findMatchingFeeds(lastExecution) {
    return mongo
        .read(nconf.get('schema').semantics, { when:
            { "$gt": new Date( lastExecution.format() ) }
        })
        .tap(function(total) {
            if(_.size(total)) {
        ...
        .tap(function(total) {
            if(_.size(total))
                debug("Found %d feeds matching with the latest `semantics`", _.size(total));
        });
};
...
module.exports = {
    ...
    findMatchingFeeds: findMatchingFeeds,
    composeXML: composeXML,
    saveRSSfile: saveRSSfile,
