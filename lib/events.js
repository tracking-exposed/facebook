...
function parseEvents(memo, evnt) {
    ...
    if(evnt.type === 'impression') {
        ...
        impression.impressionTime = new Date(evnt.impressionTime);
...
function processEvents(req) {
    ...
    debug("TL %s", JSON.stringify(processed.timelines, undefined, 2));
    debug("IMP %s", JSON.stringify(processed.impressions, undefined, 2));
    ...
    return Promise
        ...
        .tap(function(results) { 
            return mongo.upsertOne(
                ...
                { lastInfo: new Date(moment().toISOString()) }
            );
        })
