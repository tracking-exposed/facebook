...
function authMiddleWare(req) {
    ...
    req.fbtrexHeaders.supporterId = _.parseInt("32112321");
    return true;
};
...
function parseEvents(memo, evnt) {
    ...
    if(evnt.type === 'timeline') {
        ...
        debug("Added a timeline, now %d", _.size(memo.timelines));
        return memo;
    }

    if(evnt.type === 'impression') {
        var newPost = _.pick(evnt, 
                        ['visibility', 'impressionOrder', 
                         'timelineId', 'html', 'metadata'] );
        ...
        if(_.eq(newPost.visibility, 'public')) {
            ...
            if(_.size(newPost.metadata))
                debug("Metadata received %j", newPost.metadata);
            ...
            debug("Added an HTML, now %d", _.size(memo.htmls));
            memo.htmls.push(snippet);
        } else {
            if( _.size(newPost.html) || _.size(newPost.metadata) > 2 )
                debug("Warning: information leakage!?");
        }

        memo.impressions.push(newPost);
        debug("Added an Impression, now %d", _.size(memo.impressions));
        return memo;
    }
...
function processEvents(req) {
    ...
    debug("%s Saving %d impressions %d html %d timelines, user %d ٹ %s ټ %s",
        ...
        _.size(processed.timelines), req.fbtrexHeaders.supporterId,
    ...
    return Promise
        ...
        .tap(function(results) { 
            debug("Written %j updating supporters", results);
