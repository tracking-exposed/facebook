...
function reducer1(req) {
    ...
    var retVal = { impressions: [], posts: [], timewindow: timewindow  };
    var lookForTimelines = _.partial(timelineSearch, timewindow);
    ...
    return various
        ...
        .then(function() {
            if(!_.size(retVal.posts) || !_.size(retVal.impressions))
                debug("(Guessed) error: the posts are %d and the impressions %d",
                    _.size(retVal.posts), _.size(retVal.impressions) );
            return { json: retVal };
        })
        .catch(function(e) {
            debug("Error catch: %s", e.message);
