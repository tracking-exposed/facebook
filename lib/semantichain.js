...
async function saveSemantic(semantics) {
    if(!_.size(semantics))
        return null;

    try {
        return await mongo.writeMany(mongodrivers.writec, nconf.get('schema').semantics, semantics);
    } catch(error) {
...
async function saveLabel(label) {
    ...
    try {
        return await mongo.writeOne(mongodrivers.writec, nconf.get('schema').labels, label);
    } catch(error) {
...
function dandelionCheck(analyzed) { 
...
function wrapFunction(dissectorF, source, envelope) {
    try {
        ...
        _.set(envelope.log, dissectorF, resultIndicator);
        return retval;
    } catch(error) {
        _.set(envelope.log, dissectorF, "!E");
        throw error;
    }
}
