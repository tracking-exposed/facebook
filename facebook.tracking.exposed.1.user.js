...
// @version      0.1
// @description  Collection data on timeline sorting, to analyze and extract evidence in (potential) user manipoulation.
...
var newUserContent = function(jNode) {
    ...
    if (_.isNull(user)) {
        alert("Disable the script: doesn't work in your condition");
        return;
    }
...
var scrollInitialWaitTime = 2000,
...
var scrollDown = function() {
      ...
      whereScroll = _.random(0, height) + (height * refreshTimes);
      scrollTimeout += (scrollInitialWaitTime / 2); // ( _.random(5, 50) * _.random(10, 20) );
      if(scrollTimeout >= timeLimit) {
          console.log("Delay >= " + timeLimit + " => " + scrollInitialWaitTime + " reload");
          appendLog({ 'why': 'auto_refresh', 'from': location, 'when' : moment(), 'refresh_times': refreshTimes });
          location.reload();
          scrollTimeout = scrollInitialWaitTime;
          whereScroll = height;
      } else {
